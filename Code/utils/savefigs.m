...
for m = 1:length(hfigs)
    ...
    filename = strrep(get(hfigs(m),'Name'),' ','');% input('Filename? (0 to skip)\n', 's')%Prompt user
    if isempty(filename)
        filename=['Figure_',num2str(hfigs(m))];
    end
...
%         export_fig(filename,'-eps')
        %saveas(hfigs(m), [filename '.eps']) %Windows Enhanced Meta-File (best for powerpoints)
        %saveas(hfigs(m), [filename '.tiff']) %Standard PNG graphics file (best for web)
